...
def prepare_train_features(examples, tokenizer, max_length=384, doc_stride=128):
    ...
    for i, offsets in enumerate(offset_mapping):
        ...
        # Get the sequence ids to know what is question vs context
        sequence_ids = tokenized_examples.sequence_ids(i)
        
        # If no answer, set impossible answer (cls_index)
        ...
        # Check if the answer is in this chunk (not truncated)
        if not (offsets[token_start_index][0] <= start_char and 
                offsets[token_end_index][1] >= end_char):
            # Answer is outside this chunk, mark as impossible
            tokenized_examples["start_positions"].append(0)
            tokenized_examples["end_positions"].append(0)
        else:
            # Move token_start_index and token_end_index to the actual answer
